...
public class ModFlammableRotatedPillarBlock extends RotatedPillarBlock {
    ...
    public @Nullable BlockState getToolModifiedState(BlockState state, UseOnContext context, ToolAction toolAction, boolean simulate) {
        if (context.getItemInHand().getItem() instanceof AxeItem) {
            ...
            if (state.is(ModBlocks.PAULOWNIA_LOG.get())) {
                return ModBlocks.STRIPPED_PAULOWNIA_LOG.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
            if (state.is(ModBlocks.PAULOWNIA_WOOD.get())) {
                return ModBlocks.STRIPPED_PAULOWNIA_WOOD.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
        }
